const connToken = "90932070|-31949218203978968|90962394";
...
function getEmp() {
    ...
    if (jsonObj.status === 200) {
        // If Employee ID exists in the database, populate the fields and enable "Change" and "Reset" buttons
        showData(jsonObj);
        $("#eid").prop("disabled", true); // Disable only Employee ID
        disableForm(false); // Enable other form fields for editing
        $("#edit").prop("disabled",true);
        $("#save").prop("disabled", true);
        $("#change").prop("disabled", false);
        $("#reset").prop("disabled", false);
    } else {
        // If Employee ID does not exist, allow user to enter new data
        $("#eid").prop("disabled", true); // Disable Employee ID once entered
        disableForm(false); // Enable other form fields
        $("#save").prop("disabled", false); // Enable save button
        $("#reset").prop("disabled", false); // Enable reset button
        $("#ename").focus(); // Move focus to Employee Name
    }
}
